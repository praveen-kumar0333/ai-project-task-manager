 ...
 * Supports Aiven MySQL and cloud providers requiring SSL/TLS (rejectUnauthorized configurable).
 */
const sslConfig = (() => {
  const sslMode = (process.env.DB_SSL || process.env.MYSQL_SSL || '').toLowerCase();
  const isAiven = Boolean(process.env.DB_HOST && process.env.DB_HOST.includes('aivencloud.com'));

  // If DB_SSL is explicitly enabled ('true', '1', 'required') or host is Aiven
