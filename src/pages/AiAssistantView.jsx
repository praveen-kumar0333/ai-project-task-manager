    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        <div className="flex items-center gap-3">
          <div className="p-1 rounded-xl bg-indigo-500/20 border border-indigo-400/30">
            <FloatingOrb size="sm" state="active" showRings={false} />
          </div>
          <h2 className="font-extrabold text-2xl sm:text-3xl text-white tracking-tight">Google Gemini AI Assistant</h2>
        </div>
        <p className="text-sm text-slate-400 mt-1">
          Spatial developer assistant for conversational intelligence, RAG document knowledge, and task synthesis
        </p>
      </div>

      {/* Spatial Feature Selector Tabs */}
      <GlassPanel level="base" className="p-1.5 flex flex-wrap items-center gap-2">
        <button
          ...
          className={`py-2 px-3 text-xs font-semibold rounded-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer ${
            activeTab === 'chat'
              ? 'bg-indigo-500/30 text-cyan-300 border border-indigo-400/40 shadow-[0_0_12px_rgba(99,102,241,0.25)]'
              : 'text-slate-400 hover:text-slate-200 hover:bg-white/5 border border-transparent'
          }`}
        >
          <MessageSquare className="w-3.5 h-3.5" />
          <span>Chat Assistant</span>
          <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-indigo-500/30 text-cyan-300 border border-indigo-400/30">Voice &amp; Chat</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-emerald-500/30 text-emerald-300 border border-emerald-400/30">RAG</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[9px] font-bold px-1.5 py-0.2 rounded bg-purple-500/30 text-purple-300 border border-purple-400/30">Generator</span>
        </button>
        <button
        ...
        >
          ...
          <span className="text-[9px] font-medium px-1.5 py-0.2 rounded bg-slate-800 text-slate-400">Preview</span>
        </button>
        <button
      ...
      {activeTab === 'task-gen' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          <GlassPanel level="elevated" className="lg:col-span-5 p-6 space-y-4">
            <div className="border-b border-white/10 pb-3">
              <h3 className="font-bold text-base text-white">Task Breakdown Input</h3>
            ...
            <div>
              ...
              <input
                ...
                className="w-full text-sm bg-slate-900 border border-white/15 rounded-xl px-3 py-2 text-slate-200 outline-none focus:border-cyan-400"
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full text-sm bg-slate-900 border border-white/15 rounded-xl px-3 py-2 text-slate-200 outline-none focus:border-cyan-400 resize-none"
              />
            </div>
            ...
            {taskGenError && (
              <div className="p-3.5 bg-rose-500/20 border border-rose-500/40 rounded-xl flex items-start gap-2.5 text-rose-300 text-xs animate-in fade-in duration-150">
            ...
            <button
              ...
              className="w-full py-2.5 bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 text-white rounded-xl font-semibold text-sm transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-indigo-500/25 border border-white/20 active:scale-[0.98]"
            >
              {isGeneratingTasks ? (
                <>
          ...
          <GlassPanel level="elevated" className="lg:col-span-7 p-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between border-b border-white/10 pb-3 mb-4">
                <div>
                  <h3 className="font-bold text-base text-white">Generated Task Suggestions</h3>
                  <p className="text-xs text-slate-400">Review, edit, and select AI-generated items before database persistence</p>
                </div>
                {generatedTasks.length > 0 && (
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-cyan-300 bg-indigo-500/20 border border-indigo-400/30 px-2.5 py-1 rounded-lg">
                    ...
                    <button
                      ...
                      className="text-xs text-cyan-400 hover:text-cyan-300 font-semibold cursor-pointer ml-1 transition-colors"
                    >
              ...
              {taskSaveError && (
                <div className="mb-3.5 p-3.5 bg-rose-500/20 border border-rose-500/40 rounded-xl flex items-start gap-2.5 text-rose-300 text-xs animate-in fade-in duration-150">
                  <AlertCircle className="w-4 h-4 shrink-0 mt-0.5 text-rose-400" />
                  <div className="flex-1">
              ...
              {generatedTasks.length > 0 ? (
                  ...
                  {generatedTasks.map((task, idx) => {
                    ...
                    if (isEditing) {
                      return (
                        <div 
                          key={idx} 
                          className="p-3.5 rounded-xl border border-indigo-400/50 bg-indigo-950/40 space-y-2"
                        >
                          <div className="flex items-center justify-between gap-2">
                            <input
                              ...
                              className="flex-1 text-sm bg-slate-900 border border-white/20 rounded-lg px-2.5 py-1 text-white outline-none focus:border-cyan-400 font-medium"
                              placeholder="Task title"
                            />
                            <select
                              value={taskEditForm.priority}
                              onChange={(e) => setTaskEditForm({ ...taskEditForm, priority: e.target.value })}
                              className="text-xs bg-slate-900 border border-white/20 rounded-lg px-2 py-1 text-slate-200 outline-none"
                            >
                          ...
                          <textarea
                            ...
                            className="w-full text-xs bg-slate-900 border border-white/20 rounded-lg px-2.5 py-1 text-slate-300 outline-none focus:border-cyan-400 resize-none"
                          ...
                          <div className="flex justify-end gap-2 pt-1">
                            <button
                              ...
                              className="px-2.5 py-1 text-xs font-medium text-slate-400 hover:text-slate-200 bg-white/5 border border-white/10 rounded-md cursor-pointer"
                            >
                              Cancel
                            </button>
                            <button
                              ...
                              className="px-2.5 py-1 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-500 rounded-md cursor-pointer flex items-center gap-1"
                            >
                    ...
                    return (
                      <div
                        ...
                        className={`p-4 rounded-xl border transition-all cursor-pointer flex items-start gap-3 ${
                          isSelected 
                            ? 'bg-indigo-950/40 border-cyan-400/50 shadow-[0_0_15px_rgba(99,102,241,0.15)]' 
                      ...
                      >
                        <div className={`mt-0.5 w-5 h-5 rounded-md border flex items-center justify-center text-white shrink-0 ${
                          isSelected ? 'bg-indigo-600 border-cyan-400' : 'border-white/20 bg-white/5'
                        }`}>
                          ...
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-1.5 shrink-0">
                              ...
                              <button
                                ...
                                className="p-1 text-slate-400 hover:text-cyan-400 rounded hover:bg-white/5 transition-colors cursor-pointer"
                              >
                ...
                <div className="p-12 text-center flex flex-col items-center justify-center border border-dashed border-white/10 rounded-xl my-4 bg-white/2">
                  <div className="w-12 h-12 rounded-2xl bg-indigo-500/20 border border-indigo-400/30 text-cyan-400 flex items-center justify-center mb-3">
                    <Sparkles className="w-6 h-6" />
                  </div>
                  <h4 className="text-sm font-bold text-white">No tasks generated yet</h4>
            ...
            {generatedTasks.length > 0 && (
              <div className="pt-4 border-t border-white/10 flex items-center justify-between gap-3 mt-4">
                <span className="text-xs text-slate-400">
                  Target: <strong className="text-cyan-300">{projects.find(p => String(p.id) === String(taskGenProject))?.title || 'Selected Project'}</strong>
                </span>
                <button
                  onClick={handleSaveTasks}
                  disabled={isSavingTasks || selectedTaskIndices.length === 0}
                  className="px-5 py-2.5 bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 text-white rounded-xl text-sm font-semibold transition-all shadow-lg shadow-indigo-500/25 border border-white/20 flex items-center gap-2 cursor-pointer active:scale-[0.98]"
                >
                  {isSavingTasks ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
      ...
      {activeTab === 'desc-gen' && (
        <GlassPanel level="elevated" className="p-6 max-w-2xl space-y-5">
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-lg text-white">Generate Project Description</h3>
              <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-white/10">
                Preview Mode
              </span>
            </div>
          ...
          <div>
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 text-sm bg-slate-900 border border-white/15 rounded-xl px-3.5 py-2.5 text-slate-200 outline-none focus:border-cyan-400 transition-all"
              />
              <button
                onClick={handleGenerateDesc}
                disabled={isGeneratingDesc || !descProjectName.trim()}
                className="px-4 py-2.5 bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 text-white rounded-xl font-semibold text-sm transition-all flex items-center gap-2 shrink-0 cursor-pointer shadow-lg shadow-indigo-500/25 border border-white/20 active:scale-[0.98]"
              >
            ...
            <div className="p-4 bg-white/5 rounded-xl border border-white/10 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-xs text-slate-400 hover:text-cyan-400 font-medium cursor-pointer transition-colors"
                >
                  Copy text
                </button>
              </div>
              <p className="text-sm text-slate-300 leading-relaxed">{descOutput}</p>
            </div>
          )}
        </GlassPanel>
      )}
      ...
      {activeTab === 'productivity' && (
        <GlassPanel level="elevated" className="p-6 max-w-3xl space-y-6">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-lg text-white">Sprint Productivity Analysis</h3>
                <span className="text-[10px] uppercase font-semibold px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-white/10">
                  Preview Mode
                </span>
              </div>
            ...
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-indigo-500 via-indigo-600 to-purple-600 hover:from-indigo-600 hover:to-purple-700 disabled:opacity-50 text-white rounded-xl font-semibold text-sm transition-all flex items-center gap-2 cursor-pointer shadow-lg shadow-indigo-500/25 border border-white/20 active:scale-[0.98]"
            >
          ...
          {productivityReport ? (
            <div className="space-y-4">
              <div className="p-4 bg-indigo-950/40 rounded-xl border border-indigo-400/30 flex items-center justify-between">
                <div>
                  <span className="text-xs font-semibold text-cyan-300 uppercase tracking-wider">Health Index</span>
                  <p className="text-sm text-slate-300 mt-1">{productivityReport.summary}</p>
                </div>
                <span className="text-2xl font-extrabold text-cyan-300 px-3 py-1 bg-white/10 border border-white/15 rounded-xl shrink-0">
              ...
              <div className="space-y-3">
                ...
                {productivityReport.recommendations.map((rec, i) => (
                  <div key={i} className="p-4 rounded-xl border border-white/10 bg-white/5">
                    <h5 className="font-semibold text-sm text-white">{rec.title}</h5>
            ...
            <div className="p-10 border border-dashed border-white/10 rounded-xl text-center bg-white/2">
              ...
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                Click "Run Preview Audit" to have Gemini inspect your current projects and tasks for workflow bottlenecks.
              </p>
            </div>
          )}
        </GlassPanel>
