...
export default function AiTaskGeneratorModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/50 backdrop-blur-xs animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200/90 overflow-hidden flex flex-col max-h-[92vh] animate-in zoom-in-95 duration-200">
        {/* Modal Header */}
        <div className="p-5 sm:p-6 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-slate-50/80 via-white to-purple-50/30">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-tr from-indigo-600 via-indigo-700 to-purple-600 text-white rounded-2xl flex items-center justify-center shadow-md shadow-indigo-500/25">
              <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-extrabold text-base sm:text-lg text-slate-900">AI Task Generator</h3>
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-purple-100 text-purple-700 border border-purple-200/60">
                  Gemini
                </span>
              </div>
          ...
          <button 
            onClick={onClose}
            disabled={isSaving}
            className="p-2 text-slate-400 hover:text-slate-700 rounded-xl hover:bg-slate-100 disabled:opacity-50 cursor-pointer transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 sm:p-6 overflow-y-auto space-y-4 flex-1">
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
              <Layers className="w-3.5 h-3.5 text-indigo-600" /> Target Project <span className="text-rose-500">*</span>
            </label>
            {projects.length > 0 ? (
              <select
                ...
                className="w-full text-xs sm:text-sm font-medium bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all disabled:opacity-60 cursor-pointer shadow-2xs"
              >
                {projects.map(p => (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full text-xs sm:text-sm font-medium bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all disabled:opacity-60 shadow-2xs"
              />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full text-xs sm:text-sm font-medium bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 focus:bg-white transition-all resize-none disabled:opacity-60 shadow-2xs"
            />
          </div>
          ...
          {isLoading ? (
            <div className="p-6 rounded-2xl bg-gradient-to-r from-indigo-50/80 via-purple-50/60 to-indigo-50/80 border border-indigo-200/80 flex flex-col items-center justify-center text-center gap-3 animate-in fade-in duration-200">
              <div className="relative">
                <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600 to-purple-600 text-white flex items-center justify-center shadow-lg shadow-indigo-500/30 animate-pulse">
                  <Sparkles className="w-6 h-6 animate-spin" />
                </div>
              </div>
              <div>
                <p className="text-sm font-bold text-indigo-900">{loadingStepText}</p>
                <p className="text-xs text-indigo-600/80 mt-0.5 font-medium">Generating structured tasks with Google Gemini API</p>
              </div>
              <div className="w-48 bg-indigo-200/60 h-1.5 rounded-full overflow-hidden">
                <div className="bg-indigo-600 h-full rounded-full w-2/3 animate-pulse"></div>
              </div>
            </div>
          ) : (
            <button
              ...
              className="w-full py-3 bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 text-white rounded-xl font-bold text-sm transition-all shadow-md shadow-indigo-500/20 hover:shadow-lg active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer"
            >
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-xs text-indigo-600 font-bold hover:text-indigo-700 cursor-pointer px-2 py-1 rounded hover:bg-indigo-50 transition-colors"
                  >
                ...
                {generatedTasks.map((task, idx) => {
                  ...
                  if (isEditing) {
                    return (
                      <div 
                        key={idx} 
                        className="p-3.5 rounded-2xl border border-indigo-300 bg-indigo-50/40 space-y-2.5 shadow-2xs"
                      >
                        <div className="flex items-center justify-between gap-2">
                          <input
                            ...
                            className="flex-1 text-xs sm:text-sm bg-white border border-slate-300 rounded-xl px-3 py-1.5 text-slate-800 outline-none focus:border-indigo-500 font-semibold"
                            placeholder="Task title"
                          />
                          <select
                            value={editFormData.priority}
                            onChange={(e) => setEditFormData({ ...editFormData, priority: e.target.value })}
                            className="text-xs font-semibold bg-white border border-slate-300 rounded-xl px-2.5 py-1.5 text-slate-800 outline-none"
                          >
                        ...
                        <textarea
                          ...
                          className="w-full text-xs bg-white border border-slate-300 rounded-xl px-3 py-2 text-slate-700 outline-none focus:border-indigo-500 resize-none"
                        ...
                        <div className="flex justify-end gap-2 pt-1">
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-medium text-slate-600 hover:text-slate-800 bg-white border border-slate-200 rounded-lg cursor-pointer"
                          >
                            Cancel
                          </button>
                          <button
                            ...
                            className="px-3 py-1.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg cursor-pointer flex items-center gap-1 shadow-xs"
                          >
                  ...
                  return (
                    <div
                      ...
                      className={`p-3.5 rounded-2xl border transition-all duration-200 cursor-pointer flex items-start gap-3 shadow-2xs ${
                        isSelected 
                          ? 'bg-gradient-to-r from-indigo-50/70 to-purple-50/30 border-indigo-200 hover:border-indigo-300' 
                          : 'bg-white border-slate-200 opacity-55 hover:opacity-80'
                      }`}
                    >
                      <div className={`mt-0.5 w-5 h-5 rounded-lg border flex items-center justify-center text-white shrink-0 transition-colors ${
                        isSelected ? 'bg-indigo-600 border-indigo-600 shadow-2xs' : 'border-slate-300 bg-white'
                      }`}>
                        {isSelected && <Check className="w-3.5 h-3.5 stroke-[3]" />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <p className="text-xs sm:text-sm font-bold text-slate-900 truncate">{task.title}</p>
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <button
                              ...
                              className="p-1 text-slate-400 hover:text-indigo-600 rounded-lg hover:bg-white transition-colors cursor-pointer"
                            >
        ...
        <div className="p-4 sm:p-5 bg-slate-50 border-t border-slate-100 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            <button
              onClick={onClose}
              disabled={isSaving}
              className="px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-slate-600 hover:text-slate-800 rounded-xl hover:bg-slate-200/60 cursor-pointer disabled:opacity-50 transition-colors"
            >
              Cancel
            </button>
            <button
              ...
              className="px-4 sm:px-5 py-2.5 text-xs sm:text-sm font-bold bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-600 hover:from-indigo-700 hover:to-purple-700 disabled:opacity-50 text-white rounded-xl transition-all shadow-md shadow-indigo-500/20 active:scale-[0.98] flex items-center gap-2 cursor-pointer"
            >
              {isSaving ? (
                <>
